action :extract_only do
  tar_path = zabbix_part_tar_path(new_resource.code_dir, new_resource.branch, new_resource.version)
...
action :install_server do
  ...
  unless ::File.exist?(::File.join(source_dir, 'already_built'))
    ...
    script "install_zabbix_part_server_#{zabbix_part_source_identifier(new_resource.branch, new_resource.version)}" do
      interpreter 'bash'
      user 'root'
      code <<-EOH
...
action :install_agent do
  ...
  unless ::File.exist?(::File.join(source_dir, 'already_built'))
    ...
    script "install_zabbix_part_agent_#{zabbix_part_source_identifier(new_resource.branch, new_resource.version)}" do
      interpreter 'bash'
      user 'root'
      code <<-EOH
...
def zabbix_part_source_identifier(branch, version)
  "#{branch.gsub('%20', '-')}-#{version}"
end

def zabbix_part_tar_path(code_dir, branch, version)
  ::File.join(code_dir, "zabbix-#{zabbix_part_source_identifier(branch, version)}.tar.gz")
end
